<?php
	...
	class AvatarModel
	{
		...
		public static function getPublicUserAvatarFilePathByUserId($user_id)
		{
			...
			$query = $database->prepare("SELECT user_has_avatar FROM BDL_Users WHERE user_id = :user_id LIMIT 1");
		...
		public static function writeAvatarToDatabase($user_id)
		{
			...
			$query = $database->prepare("UPDATE BDL_Users SET user_has_avatar = TRUE WHERE user_id = :user_id LIMIT 1");
			$query->execute(array(':user_id' => $user_id));
		}
	    ...
	    public static function deleteAvatar($userId)
	    {
	        ...
	        $sth = $database->prepare("UPDATE BDL_Users SET user_has_avatar = 0 WHERE user_id = :user_id LIMIT 1");
